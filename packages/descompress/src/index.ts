...
export function decompress() {
  return peek({newLine: false, maxBuffer: 16}, async (data, swap) => {
    ...
    else if (isCompress.zst(data)) return swap(null, zst.decompressStream());
    else if (isCompress.xz(data)) return swap(null, lzma.createDecompressor());
    swap(null, new stream.PassThrough());
  });
}

export type compressAvaible = "deflate"|"gzip"|"zst"|"xz"|"passThrough";
export function compress(target: compressAvaible) {
  ...
  else if (target === "zst") return zst.compressStream();
